...
export default defineConfig(({ mode }) => {
  // Merge process.env (shell vars) with .env.local / .env.[mode].local loaded by Vite
  const localEnv = { ...env, ...loadEnv(mode, cwd(), '') }
  ...
  return {
    ...
    // DuckDB WASM worker ships a sourcemap pointing to missing sources; ignore to avoid dev warnings.
      ...
      // NOTICE: Proxy OpenAI-compatible API requests to avoid CORS issues in the browser dev environment.
      // Set VITE_DEV_API_PROXY_TARGET in your .env.local to enable.
      // Then configure your provider's Base URL in AIRI settings as: http://localhost:5173/api-proxy/v1
      // In production, the API gateway should handle CORS headers itself.
      ...(localEnv.VITE_DEV_API_PROXY_TARGET
        ? {
            proxy: {
              '/api-proxy': {
                target: localEnv.VITE_DEV_API_PROXY_TARGET,
                changeOrigin: true,
